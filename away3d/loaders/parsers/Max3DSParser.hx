...
class Max3DSParser extends ParserBase {
    ...
    private function parseVertexList():Void {
        ...
        _cur_obj.verts = ArrayUtils.Prefill( new Vector<Float>(), count * 3, 0);
    ...
    private function parseFaceList():Void {
        ...
        count = _byteData.readUnsignedShort();
        _cur_obj.indices = ArrayUtils.Prefill( new Vector<UInt>(), count * 3, 0);
    ...
    private function constructObject(obj:ObjectVO, pivot:Vector3D = null):ObjectContainer3D {
        if (obj.type == Asset3DType.MESH) {
            ...
            if (obj.uvs != null) {
                ...
                obj.uvs = ArrayUtils.Prefill( new Vector<Float>(), vertices.length * 2, 0 );
                for (i in 0...vertices.length) {
    ...
    private function applySmoothGroups(vertices:Array<VertexVO>, faces:Array<FaceVO>):Void {
        ...
        for (i in 0...numVerts) {
            if ((len = vGroups[i].length) < 1)
                continue;
            clones = ArrayUtils.Prefill(new Array<Int>(), len, 0);
			vClones[i] = clones;
            clones[0] = i;
    ...
    private function readTransform():Vector<Float> {
        var data:Vector<Float> = ArrayUtils.Prefill( new Vector<Float>(), 16 );

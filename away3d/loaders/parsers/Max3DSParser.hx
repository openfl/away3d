...
class Max3DSParser extends ParserBase {
    ...
    private function parseVertexList():Void {
        ...
        _cur_obj.verts = ArrayUtils.Prefill( new Array<Float>(), count * 3, 0);
    ...
    private function parseFaceList():Void {
        ...
        count = _byteData.readUnsignedShort();
        _cur_obj.indices = ArrayUtils.Prefill( new Array<UInt>(), count * 3, 0);
        ...
        _cur_obj.smoothingGroups = ArrayUtils.Prefill( new Array<UInt>(), count, 0 );		
    }
    ...
    private function parseUVList():Void {
        ...
        count = _byteData.readUnsignedShort();
        _cur_obj.uvs = ArrayUtils.Prefill( new Array<Float>(), count * 2, 0 );
    ...
    private function constructObject(obj:ObjectVO, pivot:Vector3D = null):ObjectContainer3D {
        if (obj.type == Asset3DType.MESH) {
            var subs:Array<ISubGeometry> = null;
            ...
            obj.verts = ArrayUtils.Prefill( new Array<Float>(), vertices.length * 3, 0 );
            for (i in 0...vertices.length) {
                obj.verts[i * 3] = vertices[i].x;
                obj.verts[i * 3 + 1] = vertices[i].y;
                obj.verts[i * 3 + 2] = vertices[i].z;
            }
            obj.indices = ArrayUtils.Prefill( new Array<UInt>(), faces.length * 3, 0 );
            ...
            if (obj.uvs != null) {
                ...
                obj.uvs = ArrayUtils.Prefill( new Array<Float>(), vertices.length * 2, 0 );
                for (i in 0...vertices.length) {
            ...
            if (pivot != null) {
                if (obj.transform != null) {
                    ...
                    var dat:Vector<Float> = obj.transform.copy();
                    dat[12] = 0;
                    dat[13] = 0;
                    dat[14] = 0;
    ...
    private function applySmoothGroups(vertices:Array<VertexVO>, faces:Array<FaceVO>):Void {
        ...
        var vGroups:Array<Array<Int>> = ArrayUtils.Prefill( new Array<Array<Int>>(), numVerts);
        for (i in 0...numVerts) {
        ...
        // clone vertices
        var vClones:Array<Array<Int>> = ArrayUtils.Prefill(new Array<Array<Int>>(), numVerts );
        var clones:Array<Int>;
        for (i in 0...numVerts) {
            if ((len = vGroups[i].length) < 1)
                continue;
            clones = ArrayUtils.Prefill(new Array<Int>(), len);
    ...
    private function readTransform():Array<Float> {
        ...
        data = ArrayUtils.Prefill( new Array<Float>(), 16 );

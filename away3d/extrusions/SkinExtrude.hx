package away3d.extrusions;
	...
	class SkinExtrude extends Mesh
	{
		...
		var _profiles:Array<Array<Vector3D>>;
		var _subdivision:UInt;
		var _centerMesh:Bool;
		var _closeShape:Bool;
		var _coverAll:Bool;
		var _flip:Bool;
		 ...
		 */
		public function new(material:MaterialBase, profiles:Array<Array<Vector3D>>, subdivision:UInt = 1, centerMesh:Bool = false, closeShape:Bool = false, coverAll:Bool = false, flip:Bool = false)
		{
		 ...
		 */
		public var profiles(get, set) : Array<Array<Vector3D>>;
		public function get_profiles() : Array<Array<Vector3D>>
		{
			return _profiles;
		}
		
		public function set_profiles(val:Array<Array<Vector3D>>) : Array<Array<Vector3D>>
		{
			_profiles = val;
			invalidateGeometry();
		}
		...
		private function buildExtrude():Void
		{
			...
			if (_profiles.length > 1 && _profiles[0].length > 1 && _profiles[1].length > 1) {
				...
				throw new Error("SkinExtrude: 1 multidimentional Array<Array<Vector3D>> with minimum 2 children Array<Vector3D>. Each child Vector must hold at least 2 Vector3D.");
		...
		private function generate():Void
		{
			...
			// For loop conversion - 			for (var i:Int = 0; i < _profiles.length - 1; ++i)
			var i:Int;
		...
		private function extrude(vectsA:Array<Vector3D>, vectsB:Array<Vector3D>, vscale:Float, indexv:Int):Void
		{
			...
			// For loop conversion - 						for (i = 0; i < vectsA.length; ++i)
			...
			for (i in 0...vectsA.length) {
				...
				// For loop conversion - 								for (j = 0; j < _subdivision + 1; ++j)
			...
			// For loop conversion - 						for (i = 0; i < vectsA.length - 1; ++i)
			...
			for (i in 0...vectsA.length - 1) {
				...
				for (j in 0..._subdivision) {
					...
					// For loop conversion - 										for (k = 0; k < 6; ++k)

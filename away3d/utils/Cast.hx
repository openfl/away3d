package away3d.utils;
	...
	class Cast
	{
		...
		private static function isHex(string:String):Bool
		{
			...
			// For loop conversion - 			for (var i:Int = 0; i < length; ++i)
			var i:Int;
			for (i in 0...length) {
		...
		public static function tryColor(data:Dynamic):UInt
		{
			...
			if (Std.is(data, String)) {
				if (data == "random")
					return cast(Math.random()*0x1000000, UInt);
		...
		public static function bitmapData(data:Dynamic):BitmapData
		{
			...
			if (Std.is(data, Bitmap)) {
				if (Reflect.hasField(data, "bitmapData")) // if (data is BitmapAsset)
		...
		public static function bitmapTexture(data:Dynamic):BitmapTexture
		{
			...
			if (Std.is(data, Class)) {
				try {
					data = Type.createInstance(data, []);
				} catch (materialError:Error) {
					data = Type.createInstance(data, [0, 0]); ///(0, 0);
				}
			}

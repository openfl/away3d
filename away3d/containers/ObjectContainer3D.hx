package away3d.containers;
	...
	{
		/** @private */
		/*arcane*/ public var _ancestorsAllowMouseEnabled:Bool;
		/*arcane*/ public var _isRoot:Bool;
		
		var _scene:Scene3D;
		var _parent:ObjectContainer3D;
		var _sceneTransform:Matrix3D;
		var _sceneTransformDirty:Bool;
		// these vars allow not having to traverse the scene graph to figure out what partition is set
		var _explicitPartition:Partition3D; // what the user explicitly set as the partition
		var _implicitPartition:Partition3D; // what is inherited from the parents if it doesn't have its own explicitPartition
		var _mouseEnabled:Bool;
		var _sceneTransformChanged:Object3DEvent;
		var _scenechanged:Object3DEvent;
		var _children:Array<ObjectContainer3D>;
		var _mouseChildren:Bool;
		var _oldScene:Scene3D;
		var _inverseSceneTransform:Matrix3D;
		var _inverseSceneTransformDirty:Bool;
		var _scenePosition:Vector3D;
		var _scenePositionDirty:Bool;
		var _explicitVisibility:Bool;
		var _implicitVisibility:Bool;
		var _listenToSceneTransformChanged:Bool;
		var _listenToSceneChanged:Bool;
		// visibility passed on from parents
		
		var _ignoreTransform:Bool;
		...
		public function get_implicitPartition() : Partition3D
		{
			return _implicitPartition;
		}
		
		public function set_implicitPartition(value:Partition3D) : Partition3D
		{
		...
		{
			if (_sceneTransformDirty || _ignoreTransform==true)
				return;
		...
		{
			return Lambda.indexOf(_children, child) >= 0;
		}
		...
		{
			Lambda.foreach(childarray, function(child:ObjectContainer3D):Bool {
				addChild(child);
				return true;
			});
		}
		...
		override public function addEventListener(type:String, listener:Event->Void, useCapture:Bool = false, priority:Int = 0, useWeakReference:Bool = false):Void
		{
		...
		override public function removeEventListener(type:String, listener:Event->Void, useCapture:Bool = false):Void
		{

package away3d.tools.helpers;
	...
	class FaceHelper
	{
		...
		private static var SPLIT:UInt = 2;
		private static var TRI:UInt = 3;
		private static var QUARTER:UInt = 4;
		
		private static var _n:Vertex = new Vertex();
		private static var _t:Vertex = new Vertex();
		
		/*Adding a face*/
		public static addFace(mesh:Mesh, v0:Vertex, v1:Vertex, v2:Vertex, uv0:UV, uv1:UV, uv2:UV, subGeomIndice:UInt):Void
		{
			...
			var vertices:Array<Float> = subGeom.vertexData || new Array<Float>();
			var normals:Array<Float> = subGeom.vertexNormalData || new Array<Float>();
			var tangents:Array<Float> = subGeom.vertexTangentData || new Array<Float>();
			...
			if (lengthVertices + 9 > LIMIT) {
				indices = Array<UInt>([0, 1, 2]);
				vertices = Array<Float>([v0.x, v0.y, v0.z, v1.x, v1.y, v1.z, v2.x, v2.y, v2.z]);
				uvs = Array<Float>([uv0.u, uv0.v, uv1.u, uv1.v, uv2.u, uv2.v]);
				normals = Array<Float>([_n.x, _n.y, _n.z, _n.x, _n.y, _n.z, _n.x, _n.y, _n.z]);
				tangents = Array<Float>([_t.x, _t.y, _t.z, _t.x, _t.y, _t.z, _t.x, _t.y, _t.z]);
				subGeom = new SubGeometry();
				mesh.geometry.addSubGeometry(subGeom);
				
			} else {
				
				indices = subGeom.indexData || new Array<UInt>();
				uvs = subGeom.UVData || new Array<Float>();
		 ...
		 */
		public static removeFace(mesh:Mesh, index:UInt, subGeomIndice:UInt):Void
		{
			var pointer:UInt = index*3;
			var subGeom:SubGeometry = getSubGeometry(mesh, subGeomIndice);
			
			var indices:Array<UInt> = Lambda.array(subGeom.indexData);
			...
			var vertices:Array<Float> = Lambda.array(subGeom.vertexData);
			var normals:Array<Float> = Lambda.array(subGeom.vertexNormalData);
			var tangents:Array<Float> = Lambda.array(subGeom.vertexTangentData);
			var uvs:Array<Float> = Lambda.array(subGeom.UVData);
		 ...
		 */
		public static removeFaces(mesh:Mesh, indices:Array<UInt>, subGeomIndices:Array<UInt>):Void
		{
		 ...
		 */
		public static addFaces(mesh:Mesh, v0s:Array<Vertex>, v1s:Array<Vertex>, v2s:Array<Vertex>, uv0s:Array<UV>, uv1s:Array<UV>, uv2s:Array<UV>, subGeomIndices:Array<UInt>):Void
		{
			// For loop conversion - 			for (var i:UInt = 0; i < v0s.length; ++i)
			var i:UInt = 0;
		 ...
		 */
		public static splitFace(mesh:Mesh, indice:UInt, subGeomIndice:UInt, side:UInt = 0):Void
		{
		 ...
		 * @param    subGeomIndice            The index of the subgeometry holder this face.
		 */
		public static triFace(mesh:Mesh, indice:UInt, subGeomIndice:UInt):Void
		{
			var pointer:UInt = indice*3;
			var subGeom:SubGeometry = getSubGeometry(mesh, subGeomIndice);
			var indices:Array<UInt> = Lambda.array(subGeom.indexData);
		 ...
		 * @param    subGeomIndice            The index of the subgeometry holder this face.
		 */
		public static quarterFace(mesh:Mesh, indice:UInt, subGeomIndice:UInt):Void
		{
			var pointer:UInt = indice*3;
			var subGeom:SubGeometry = getSubGeometry(mesh, subGeomIndice);
			var indices:Array<UInt> = Lambda.array(subGeom.indexData);
		 ...
		 */
		public static splitFaces(mesh:Mesh):Void
		{
		 ...
		 */
		public static triFaces(mesh:Mesh):Void
		{
		 ...
		 */
		public static quarterFaces(mesh:Mesh):Void
		{
		...
		public static getFaceTangent(v0:Vertex, v1:Vertex, v2:Vertex, uv0V:Float, uv1V:Float, uv2V:Float, uvScaleV:Float = 1, out:Vertex = null):Vertex
		{
		...
		private static function applyMethod(methodID:UInt, mesh:Mesh, value:Float = 0):Void
		{
			...
			var faceIndex:UInt;
			var j:UInt;
			// For loop conversion - 			for (var i:UInt = 0; i < subGeoms.length; ++i)
			var i:UInt = 0;
			for (i in 0...subGeoms.length) {
				indices = subGeoms[i].indexData;
				faceIndex = 0;
				// For loop conversion - 				for (j = 0; j < indices.length; j += 3)
		...
		private static function updateSubGeometryData(subGeometry:SubGeometry, vertices:Array<Float>, indices:Array<UInt>, uvs:Array<Float>, normals:Array<Float> = null, tangents:Array<Float> = null):Void
		{
		...
		private static function getUsedIndice(vertices:Array<Float>, x:Float, y:Float, z:Float):Int
		{
			// For loop conversion - 			for (var i:UInt = 0; i < vertices.length; i += 3)
			var i:UInt = 0;

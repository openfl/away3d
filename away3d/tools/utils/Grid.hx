package away3d.tools.utils;
	...
	class Grid
	{
		...
		function Grid(unit:Float = 1, objectSpace:Bool = false):Void
		{
		...
		{
			// For loop conversion - 			for (var i:UInt = 0; i < vertices.length; ++i)
			var i:UInt = 0;
		 ...
		 */
		public function set_unit(val:Float) : Void
		{
			_unit = Math.abs(val);
			_unit = (_unit == 0)? .001 : _unit;
		}
		
		public var unit(get, set) : Void;
		
		public function get_unit() : Void
		{
			return _unit;
		}
		...
		private function parse(object3d:ObjectContainer3D, dovert:Bool = true):Void
		{
			...
			// For loop conversion - 						for (var i:UInt = 0; i < object3d.numChildren; ++i)
		...
		private function snap(mesh:Mesh):Void
		{
			...
			for (i in 0...numSubGeoms) {
				...
				// For loop conversion - 								for (j = subGeom.vertexOffset; j < vecLength; j += stride)
				
				for (j in subGeom.vertexOffset...vecLength) {

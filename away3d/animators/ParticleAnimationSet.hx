package away3d.animators;
	...
	{
		/** @private */
		/*arcane*/ public var _animationRegisterCache:AnimationRegisterCache;
		
		//all other nodes dependent on it
		var _timeNode:ParticleTimeNode;
		...
		var _animationSubGeometries:Dictionary = new Dictionary(true);
		var _particleNodes:Array<ParticleNodeBase> = new Array<ParticleNodeBase>();
		var _localDynamicNodes:Array<ParticleNodeBase> = new Array<ParticleNodeBase>();
		var _localStaticNodes:Array<ParticleNodeBase> = new Array<ParticleNodeBase>();
		var _totalLenOfOneVertex:Int = 0;
		 ...
		 */
		public var initParticleFunc:xxx->yyy;
		 ...
		 */
		public var particleNodes(get, null) : Array<ParticleNodeBase>;
		...
		override public function addAnimation(node:AnimationNodeBase):Void
		{
			...
			// For loop conversion - 						for (i = _particleNodes.length - 1; i >= 0; i--)
		...
		public function deactivate(stage3DProxy:Stage3DProxy, pass:MaterialPassBase):Void
		{
			...
			// For loop conversion - 			for (var i:Int = offset; i < used; i++)
			var i:Int;
		...
		public function getAGALVertexCode(pass:MaterialPassBase, sourceRegisters:Array<String>, targetRegisters:Array<String>, profile:String):String
		{
			...
			if (pass.animationRegisterCache==null) pass.animationRegisterCache = new AnimationRegisterCache(profile);
			_animationRegisterCache = pass.animationRegisterCache;
			...
			while (i < numParticles) {
				...
				while (j < particlesLength && (particle = particles[j]).particleIndex == i) {
					...
					for each (localNode in _localStaticNodes) {
						...
						// For loop conversion - 						for (counterForVertex = 0; counterForVertex < vertexLength; counterForVertex += _totalLenOfOneVertex)
						for (counterForVertex in 0...vertexLength) {
							vertexOffset = oneDataOffset + counterForVertex;
							
							//add the data for the local node to the vertex data
							// For loop conversion - 							for (counterForOneData = 0; counterForOneData < oneDataLen; counterForOneData++)

package away3d.animators.states;
	...
	{
		var _particleFollowNode:ParticleFollowNode;
		var _followTarget:Object3D;
		
		var _targetPos:Vector3D = new Vector3D;
		var _targetEuler:Vector3D = new Vector3D;
		var _prePos:Vector3D;
		var _preEuler:Vector3D;
		var _smooth:Bool;
			...
			for (i in 0...len) {
				...
				if (t - deltaTime <= 0) {
					...
					if (vertexData[inc] != interpolatedPos.x || vertexData[inc + 1] != interpolatedPos.y || vertexData[inc + 2] != interpolatedPos.z) {
						changed = true;
						// For loop conversion - 						for (var j:UInt = 0; j < data[i].numVertices; j++)
						var j:UInt;

package away3d.animators;
	...
	{
		var _globalMatrices:Array<Float>;
		var _globalPose:SkeletonPose = new SkeletonPose();
		var _globalPropertiesDirty:Bool;
		var _numJoints:UInt;
		var _animationStates:Dictionary = new Dictionary();
		var _condensedMatrices:Array<Float>;
		
		var _skeleton:Skeleton;
		var _forceCPU:Bool;
		var _useCondensedIndices:Bool;
		var _jointsPerVertex:UInt;
		var _activeSkeletonState:ISkeletonAnimationState;
		 ...
		 * @see #globalPose
		 */
		public var globalMatrices(get, null) : Array<Float>;
		...
		public function new(animationSet:SkeletonAnimationSet, skeleton:Skeleton, forceCPU:Bool = false)
		{
			...
			_globalMatrices = ArrayUtils.Prefill(new Array<Float>(), _numJoints*12, true);
			
			var j:Int;
			// For loop conversion - 			for (var i:UInt = 0; i < _numJoints; ++i)
			var i:UInt = 0;
		...
		public function setRenderState(stage3DProxy:Stage3DProxy, renderable:IRenderable, vertexConstantOffset:Int, vertexStreamOffset:Int, camera:Camera3D):Void
		{
			...
			if (_useCondensedIndices) {
				...
				if (_animationSet.usesCPU) {
					if (!_animationStates[skinnedGeom]) _animationStates[skinnedGeom] = new SubGeomAnimationState(skinnedGeom);
		...
		private function updateCondensedMatrices(condensedIndexLookUp:Array<UInt>, numJoints:UInt):Void
		{
		...
		private function updateGlobalProperties():Void
		{
			...
			var mtxOffset:UInt;
			var globalPoses:Array<JointPose> = _globalPose.jointPoses;
			var raw:Array<Float>;
		...
		private function morphGeometry(state:SubGeomAnimationState, subGeom:SkinnedSubGeometry):Void
		{
			...
			var jointIndices:Array<Float> = subGeom.jointIndexData;
		...
		private function localToGlobalPose(sourcePose:SkeletonPose, targetPose:SkeletonPose, skeleton:Skeleton):Void
		{
			...
			for (i in 0...len) {
				if (!globalPoses[i]) globalPoses[i] = new JointPose();
...
class SubGeomAnimationState
{
	...
	{
		animatedVertexData = Lambda.array(subGeom.vertexData);
	}

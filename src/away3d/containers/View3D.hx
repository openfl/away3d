...
class View3D  extends EventDispatcher {
    ...
    public var layeredView(get_layeredView, set_layeredView):Bool;
    public var filters3d(get_filters3d, set_filters3d):Array<Dynamic>;
    ...
    public var deltaTime(get_deltaTime, never):Int;
    public var antiAlias(get_antiAlias, set_antiAlias):Int;
    public var renderedFacesCount(get_renderedFacesCount, never):Int;
    ...
    public function new(scene:Scene3D = null, camera:Camera3D = null, renderer:RendererBase = null, forceSoftware:Bool = false, profile:String = "baseline") {
        ...
        _renderer = renderer;
        if (_renderer == null)_renderer = new DefaultRenderer();
...
// todo: entity collector should be defined by renderer
    ...
    public function set_stage3DProxy(stage3DProxy:Stage3DProxy):Stage3DProxy {
        if (_stage3DProxy != null) _stage3DProxy.removeEventListener(Stage3DEvent.VIEWPORT_UPDATED, onViewportUpdated);
...
/**
    ...
    public function set_camera(camera:Camera3D):Camera3D {
        ...
        _camera = camera;
        _entityCollector.camera = _camera;
        if (_scene != null) _camera.partition = _scene.partition;
        _camera.addEventListener(CameraEvent.LENS_CHANGED, onLensChanged);
        _scissorRectDirty = true;
        _viewportDirty = true;
        return camera;
...
/**
    ...
    public function set_scene(scene:Scene3D):Scene3D {
        ...
        _scene = scene;
        _scene.addEventListener(Scene3DEvent.PARTITION_CHANGED, onScenePartitionChanged);
        if (_camera != null) _camera.partition = _scene.partition;
        return scene;
    }
...
/**
    ...
    public function set_antiAlias(value:Int):Int {
        _antiAlias = value;
...
/**
    ...
    public function get_renderedFacesCount():Int {
...
/**
    ...
    public function set_shareContext(value:Bool):Bool {
        if (_shareContext == value) return value;
        _shareContext = value;
        _globalPosDirty = true;
        return value;
    }
...
//if context3D has Disposed by the OS,don't render at this frame
        ...
        if (_shareContext && _layeredView) stage3DProxy.clearDepthBuffer();
        if (!_parentIsStage) {
        ...
        if (_globalPosDirty) updateGlobalPos();
        updateTime();
        updateViewSizeData();
...
// update picking
        ...
        if (_depthPrepass) renderDepthPrepass(_entityCollector);
...
// register that a view has been rendered
    ...
    private function renderSceneDepthToTexture(entityCollector:EntityCollector):Void {
        if (_depthTextureInvalid || _depthRender == null) initDepthTexture(_stage3DProxy._context3D);
        _depthRenderer.textureRatioX = _rttBufferManager.textureRatioX;
        _depthRenderer.textureRatioY = _rttBufferManager.textureRatioY;
    ...
    private function initDepthTexture(context:Context3D):Void {
        ...
        if (_depthRender != null) _depthRender.dispose();
...
/**
    ...
    public function dispose():Void {
        ...
        if (!shareContext) _stage3DProxy.dispose();
        _renderer.dispose();
        if (_depthRender != null) _depthRender.dispose();
        if (_rttBufferManager != null) _rttBufferManager.dispose();

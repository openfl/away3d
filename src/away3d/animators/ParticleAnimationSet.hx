...
//reset animationRegisterCache
        ...
        for (node in _particleNodes) {
            if (node.priority < POST_PRIORITY) code += node.getAGALVertexCode(pass, _animationRegisterCache);
        }

        code += _animationRegisterCache.getCombinationCode();
        for (node in _particleNodes) {
            if (node.priority >= POST_PRIORITY && node.priority < COLOR_PRIORITY) code += node.getAGALVertexCode(pass, _animationRegisterCache);
        }

        code += _animationRegisterCache.initColorRegisters();
        for (node in _particleNodes) {
            if (node.priority >= COLOR_PRIORITY) code += node.getAGALVertexCode(pass, _animationRegisterCache);
        }
...
/**
    ...
    public function getAGALUVCode(pass:MaterialPassBase, UVSource:String, UVTarget:String):String {
        ...
        if (hasUVNode) {
            ...
            var node:ParticleNodeBase;
            for (node in _particleNodes)code += node.getAGALUVCode(pass, _animationRegisterCache);

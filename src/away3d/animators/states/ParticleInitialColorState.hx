...
// TODO: not used
        ...
        if (animationRegisterCache.needFragmentAnimation) {
            if (_particleInitialColorNode.mode == ParticlePropertiesMode.LOCAL_STATIC) {
                ...
                if (_usesOffset) animationSubGeometry.activateVertexBuffer(animationRegisterCache.getRegisterIndex(_animationNode, ParticleInitialColorNode.OFFSET_INDEX), dataOffset, stage3DProxy, Context3DVertexBufferFormat.FLOAT_4);
            }

            else {
                if (_usesMultiplier) animationRegisterCache.setVertexConst(animationRegisterCache.getRegisterIndex(_animationNode, ParticleInitialColorNode.MULTIPLIER_INDEX), _multiplierData.x, _multiplierData.y, _multiplierData.z, _multiplierData.w);
                if (_usesOffset) animationRegisterCache.setVertexConst(animationRegisterCache.getRegisterIndex(_animationNode, ParticleInitialColorNode.OFFSET_INDEX), _offsetData.x, _offsetData.y, _offsetData.z, _offsetData.w);
            }
    ...
    private function updateColorData():Void {
        if (_particleInitialColorNode.mode == ParticlePropertiesMode.GLOBAL) {
            if (_usesMultiplier) _multiplierData = new Vector3D(_initialColor.redMultiplier, _initialColor.greenMultiplier, _initialColor.blueMultiplier, _initialColor.alphaMultiplier);
            if (_usesOffset) _offsetData = new Vector3D(_initialColor.redOffset / 255, _initialColor.greenOffset / 255, _initialColor.blueOffset / 255, _initialColor.alphaOffset / 255);
        }
    }

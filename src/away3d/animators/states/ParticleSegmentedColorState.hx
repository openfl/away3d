...
/**
    ...
    override public function setRenderState(stage3DProxy:Stage3DProxy, renderable:IRenderable, animationSubGeometry:AnimationSubGeometry, animationRegisterCache:AnimationRegisterCache, camera:Camera3D):Void {
        if (animationRegisterCache.needFragmentAnimation) {
            ...
            if (_usesOffset) animationRegisterCache.setVertexConstFromVector(animationRegisterCache.getRegisterIndex(_animationNode, ParticleSegmentedColorNode.START_OFFSET_INDEX), _offsetData);
        }
    }
    ...
    private function updateColorData():Void {
        ...
        while (i < _numSegmentPoint) {
            if (i == 0) _timeLifeData.push(_segmentPoints[i].life)
            else _timeLifeData.push(_segmentPoints[i].life - _segmentPoints[i - 1].life);
            i++;
        }

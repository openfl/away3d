...
/**
        ...
        if (animationRegisterCache.needUVAnimation) {
            ...
            if (_usesCycle) {
                ...
                if (_particleSpriteSheetNode.mode == ParticlePropertiesMode.LOCAL_STATIC) {
                    if (_usesPhase) animationSubGeometry.activateVertexBuffer(index, _particleSpriteSheetNode.dataOffset, stage3DProxy, Context3DVertexBufferFormat.FLOAT_3)
                    else animationSubGeometry.activateVertexBuffer(index, _particleSpriteSheetNode.dataOffset, stage3DProxy, Context3DVertexBufferFormat.FLOAT_2);
                }

                else animationRegisterCache.setVertexConst(index, _spriteSheetData[4], _spriteSheetData[5]);
            }
        }
    }
    ...
    private function updateSpriteSheetData():Void {
        ...
        if (_usesCycle) {
            ...
            if (_usesPhase) _spriteSheetData[6] = _cyclePhase;
        }
    }

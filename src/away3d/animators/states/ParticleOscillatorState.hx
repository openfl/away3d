...
/**
    ...
    override public function setRenderState(stage3DProxy:Stage3DProxy, renderable:IRenderable, animationSubGeometry:AnimationSubGeometry, animationRegisterCache:AnimationRegisterCache, camera:Camera3D):Void {
        ...
        if (_particleOscillatorNode.mode == ParticlePropertiesMode.LOCAL_STATIC) animationSubGeometry.activateVertexBuffer(index, _particleOscillatorNode.dataOffset, stage3DProxy, Context3DVertexBufferFormat.FLOAT_4)
        else animationRegisterCache.setVertexConst(index, _oscillatorData.x, _oscillatorData.y, _oscillatorData.z, _oscillatorData.w);
    }

    private function updateOscillatorData():Void {
        if (_particleOscillatorNode.mode == ParticlePropertiesMode.GLOBAL) {
            if (_oscillator.w <= 0) throw (new Error("the cycle duration must greater than zero"));

package away3d.loaders.parsers;
	...
	class AWD1Parser extends ParserBase
	{
		...
		var _objs:Array<Dynamic>;
		var _geos:Array<Dynamic>;
		var _oList:Array<Dynamic>;
		var _aC:Array<Dynamic>;
		var _dline:Array<Dynamic>;
		...
		private override function proceedParsing():Bool
		{
			...
			while (_charIndex < _stringLength && hasTime()) {
				...
				if (_state == "#o") {
					if (_buffer == 0) {
						_id = _dline[0];
						m = new Matrix3D(Array<Float>([parseFloat(_dline[1]), parseFloat(_dline[5]), parseFloat(_dline[9]), 0, parseFloat(_dline[2]), parseFloat(_dline[6]), parseFloat(_dline[10]), 0, parseFloat(_dline[3]), parseFloat(_dline[7]), parseFloat(_dline[11]), 0, parseFloat(_dline[4]), parseFloat(_dline[8]), parseFloat(_dline[12]), 1]));
			...
			if (_charIndex >= _stringLength) {
				...
				// For loop conversion - 								for (i = 0; i < _objs.length; ++i)
		...
		private function parseFacesToMesh(geo:Object, mesh:Mesh):Void
		{
			...
			// For loop conversion - 						for (j = 0; j < aRef.length; j += 6)
			
			for (j in 0...aRef.length) {
		...
		{
			// For loop conversion - 			for (var i:Int = 0; i < _meshList.length; ++i)
			var i:Int;
		...
		private function read(str:String):String
		{
			...
			// For loop conversion - 			for (var i:Int = 0; i < charcount; ++i)
			var i:Int;

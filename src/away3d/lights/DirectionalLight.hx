package away3d.lights;
	...
	class DirectionalLight extends LightBase
	{
		...
		var _tmpLookAt:Vector3D;
		var _sceneDirection:Vector3D;
		var _projAABBPoints:Array<Float>;
		...
		override public function getObjectProjectionMatrix(renderable:IRenderable, target:Matrix3D = null):Matrix3D
		{
			...
			if (_projAABBPoints==null)
				_projAABBPoints = new Array<Float>();
			...
			var d:Float;
			// For loop conversion - 			for (var i:Int = 0; i < 24; )
			var i:Int = 0;
			while (i < 24) {
				d = _projAABBPoints[i++];
				if (d < xMin)

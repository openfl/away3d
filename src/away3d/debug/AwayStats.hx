package away3d.debug;
	...
	class AwayStats extends Sprite
	{
		...
		private static var _WIDTH:Int = 125;
		private static var _MAX_HEIGHT:Int = 85;
		private static var _MIN_HEIGHT:Int = 51;
		private static var _UPPER_Y:Float = -1;
		private static var _MID_Y:Float = 9;
		private static var _LOWER_Y:Float = 19;
		private static var _DIAG_HEIGHT:Int = _MAX_HEIGHT - 50;
		private static var _BOTTOM_BAR_HEIGHT:Int = 31;
		
		private static var _POLY_COL:UInt = 0xffcc00;
		private static var _MEM_COL:UInt = 0xff00cc;
		...
		public var max_ram(get, null):Float;
		private function get_max_ram():Float
		{
			return _max_ram;
		}
		
		public var ram(get, null) : Float;
		private function get_ram() : Float
		{
			return _ram;
		}
		
		public var avg_fps(get, null):Float;
		private function get_avg_fps():Float
		{
			return _avg_fps;
		}
		
		public var max_fps(get, null):UInt;
		 ...
		 */
		public static var instance(get, null):AwayStats;
		...
		{
			if (view3d!=null && Lambda.indexOf(_views, view3d) < 0)
				_views.push(view3d);
		}
		...
		{
			if (view3d!=null) {
				var idx:Int = Lambda.indexOf(_views, view3d);
				if (idx >= 0)
					_views.splice(idx, 1);
			}
		}
		...
		private function _initMisc():Void
		{
			...
			if (_mean_data_length > 0) {
				...
				_mean_data = [];
				// For loop conversion - 				for (i = 0; i < _mean_data_length; i++)
		...
		private function _redrawStats():Void
		{
			...
			// Redraw counters
			_fps_tf.text = Std.string(_fps) + '/' + Std.int(stage.frameRate);
		...
		private function _redrawMemGraph():Void
		{
			...
			// For loop conversion - 			for (i = _mem_points.length - 1; i >= 0; --i)
		...
		private function _getRamString(ram:Float):String
		{
			...
			return Std.int(ram*10)/10 + ram_unit;
		}
		...
		public function reset():Void
		{
			...
			// Reset RAM usage log
			// For loop conversion - 			for (i = 0; i < _WIDTH/5; i++)
			for (i in 0...Std.int(_WIDTH/5))
				_mem_points[i] = 0;
			
			// Reset FPS log if any
			if (_mean_data!=null) {
				// For loop conversion - 				for (i = 0; i < _mean_data.length; i++)
		...
		private function _onTimer(ev:Event):Void
		{
			...
			if (_views.length > 0) {
				...
				// For loop conversion - 				for (i = 0; i < _views.length; i++)

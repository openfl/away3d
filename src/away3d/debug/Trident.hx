package away3d.debug;
	...
	class Trident extends Mesh
	{
		...
		private function buildTrident(length:Float, showLetters:Bool):Void
		{
			...
			var vectors:Array<Array<Vector3D>> = new Array<Array<Vector3D>>();
			var colors:Array<UInt> = Array<UInt>([0xFF0000, 0x00FF00, 0x0000FF]);
			...
			var profileO:Array<Vector3D> = new Array<Vector3D>();
			...
			merge.applyToMeshes(this, Array<Mesh>([arrowX, arrowY, arrowZ, origin]));
			...
			if (showLetters) {
				...
				//x
				vectors[3] = Array<Vector3D>([    new Vector3D(length + scl2, scl1, 0),
					new Vector3D(length + scl3, -scl1, 0),
					new Vector3D(length + scl3, scl1, 0),
					new Vector3D(length + scl2, -scl1, 0)]);
				//y
				vectors[4] = Array<Vector3D>([    new Vector3D(-scaleW*1.2, length + scl4, 0),
				...
				//z
				vectors[5] = Array<Vector3D>([    new Vector3D(0, scl1, length + scl2),

...
class SimplePanVolumeDriver extends AbstractSound3DDriver implements ISound3DDriver {
    ...
    public function play():Void {
        var pos:Float;
        if (_src == null) throw new Error("SimplePanVolumeDriver.play(): No sound source to play.");
        _playing = true;
...
// Only update sound transform while playing
        if (_playing) _updateSoundTransform();
    }

    private function _updateSoundTransform():Void {
        _st3D.updateFromVector3D(_ref_v);
        if (_sound_chan != null) _sound_chan.soundTransform = _st3D.soundTransform;
    }

package away3d.materials.passes;
	...
	{
		private static var ONE_VECTOR:Array<Float> = [ 1, 1, 1, 1 ];
		private static var FRONT_VECTOR:Array<Float> = [ 0, 0, -1, 0 ];
		
		var _constants:Array<Float>;
		var _calcMatrix:Matrix3D;
		...
		override function render(renderable:IRenderable, stage3DProxy:Stage3DProxy, camera:Camera3D, viewProjection:Matrix3D):Void
		{
			...
			if (cast(renderable, SegmentSet).hasData) {
				// For loop conversion - 				for (var i:UInt = 0; i < subSetCount; ++i)
				var i:UInt = 0;
				for (i in 0...subSetCount) {
					renderable.activateVertexBuffer(i, stage3DProxy);
					context.setProgramConstantsFromMatrix(Context3DProgramType.VERTEX, 8, _calcMatrix.flashMatrix3D, true);
		...
		override public function activate(stage3DProxy:Stage3DProxy, camera:Camera3D):Void
		{
			...
			// projection matrix
			context.setProgramConstantsFromMatrix(Context3DProgramType.VERTEX, 0, camera.lens.matrix.flashMatrix3D, true);
		}

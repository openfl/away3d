...
class SegmentPass extends MaterialPassBase {
    ...
    static private var ONE_VECTOR:Vector<Float> = Vector.ofArray(cast [1, 1, 1, 1]);
    static private var FRONT_VECTOR:Vector<Float> = Vector.ofArray(cast [0, 0, -1, 0]);
...
/**
    ...
    override public function activate(stage3DProxy:Stage3DProxy, camera:Camera3D):Void {
        ...
        if (stage3DProxy.scissorRect != null) _constants[0] = _thickness / Math.min(stage3DProxy.scissorRect.width, stage3DProxy.scissorRect.height)
        else _constants[0] = _thickness / Math.min(stage3DProxy.width, stage3DProxy.height);

package away3d.library;
	...
	class AssetLibraryBundle extends EventDispatcher
	{
		...
		var _strategy:ConflictStrategyBase;
		var _strategyPreference:String;
		
		var _assets:Array<IAsset>;
		var _assetDictionary:StringMap<StringMap<IAsset>>;
		var _assetDictDirty:Bool;
		...
		public static function getInstance(?key:String = 'default'):AssetLibraryBundle
		{
			...
			if (!AssetLibrary._instances.exists(key))
				AssetLibrary._instances.set(key, new AssetLibraryBundle(new AssetLibraryBundleSingletonEnforcer()));
		 ...
		 *
		 */
		public function enableParser(parserClass):Void
		{
		 ...
		 */
		public function createIterator(assetTypeFilter:String = null, namespaceFilter:String = null, filterFunc:IAsset->Bool = null):AssetLibraryIterator
		{
		...
		public function getAsset(name:String, ns:String = null):IAsset
		{
			...
			if (ns==null) ns = NamedAssetBase.DEFAULT_NAMESPACE;
...
trace("AssetLibraryBundle.addAsset:indexOf="+Lambda.indexOf(_assets, asset));
			if (Lambda.indexOf(_assets, asset) >= 0)
				return;
			
			old = getAsset(asset.name, asset.assetNamespace);
			ns = asset.assetNamespace!=null ? asset.assetNamespace : NamedAssetBase.DEFAULT_NAMESPACE;
trace("AssetLibraryBundle.addAsset:old="+old+" name="+asset.name+" ns:"+asset.assetNamespace+" asset:"+asset);

			if (old != null)
				_strategy.resolveConflict(asset, old, _assetDictionary.get(ns), _strategyPreference);

trace("AssetLibraryBundle.addAsset:after resolveConflict="+asset.name);
			...
			// Add it
			_assets.push(asset);
trace("AssetLibraryBundle.addAsset:assetsLen="+_assets.length+" asset.id="+asset.id);
			if (!_assetDictionary.exists(ns))
				_assetDictionary.set(ns, new StringMap<IAsset>());
trace("AssetLibraryBundle.addAsset:set ns="+ns+" asset="+asset.name+" asset="+asset);
		...
		public function removeAsset(asset:IAsset, dispose:Bool = true):Void
		{
			...
			idx = Lambda.indexOf(_assets, asset);
			if (idx >= 0)
		...
		public function removeNamespaceAssets(ns:String = null, dispose:Bool = true):Void
		{
			...
			old_assets = Lambda.array(_assets);
			_assets = [];
		...
		private function removeAssetFromDict(asset:IAsset, autoRemoveEmptyNamespace:Bool = true):Void
		{
			...
			if (_assetDictionary.exists(asset.assetNamespace)) {
				...
				if (autoRemoveEmptyNamespace) {
					...
					for (key in _assetDictionary.get(asset.assetNamespace).keys()) {
						empty = false;
						break;
					}
...
trace("AssetLibraryBundle.onAssetComplete:name:"+event.asset.name+" event.type="+Std.string(event.type)+" asset:"+Type.getClassName(Type.getClass(event.asset)));
			if (event.type == AssetEvent.ASSET_COMPLETE)
				addAsset(event.asset);
if (Type.getClassName(Type.getClass(event.asset)) == "away3d.textures.BitmapTexture") {
	var b:BitmapTexture = cast(event.asset, BitmapTexture);
	trace(" - BitmapTexture="+b.width+"/"+b.height);
}		
		...
		private function onResourceRetrieved(event:LoaderEvent):Void
		{
			...
			var index:Int = Lambda.indexOf(_loadingSessions, loader);

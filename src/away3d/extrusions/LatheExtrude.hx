package away3d.extrusions;
	...
	{
		private var EPS:Float = .0001;
		private var LIMIT:UInt = 196605;
		private var MAXRAD:Float = 1.2;
		
		var _profile:Array<Vector3D>;
		var _lastProfile:Array<Vector3D>;
		var _keepLastProfile:Bool;
		var _axis:String;
		var _revolutions:Float;
		var _subdivision:UInt;
		var _offsetRadius:Float;
		var _prevOffsetRadius:Float = 0;
		var _materials:MultipleMaterials;
		var _coverAll:Bool;
		var _flip:Bool;
		var _centerMesh:Bool;
		var _thickness:Float;
		var _preciseThickness:Bool;
		var _ignoreSides:String;
		var _smoothSurface:Bool;
		var _tweek:Object;
		var _varr:Array<Vector3D>;
		var _varr2:Array<Vector3D>;
		var _uvarr:Array<UV>;
		var _startRotationOffset:Float = 0;
		 ...
		 */
		public function new(material:MaterialBase = null, profile:Array<Vector3D> = null, axis:String = LatheExtrude.Y_AXIS, revolutions:Float = 1, subdivision:UInt = 10, coverall:Bool = true, centerMesh:Bool = false, flip:Bool = false, thickness:Float = 0, preciseThickness:Bool = true, offsetRadius:Float = 0, materials:MultipleMaterials = null, ignoreSides:String = "", tweek:Object = null, smoothSurface:Bool = true)
		{
		...
		/*
		 * A Array<Vector3D> representing the profile information to be repeated/rotated around a given axis.
		 */
		...
		public function set_profile(val:Array<Vector3D>) : Array<Vector3D>
		{
			if (val.length > 1) {
				...
				throw new Error("LatheExtrude error: the profile Array<Vector3D> must hold a mimimun of 2 vector3D's");
		}
		 ...
		 * @see #revolutions
		 */
		public var subdivision(get, set) : UInt;
		public function get_subdivision() : UInt
		{
			return _subdivision;
		}
		
		public function set_subdivision(val:UInt) : UInt
		{
			val = (val < 3)? 3 : val;
			if (_subdivision == val)
				return;
		 ...
		 */
		public var lastProfile(get, null) : Array<Vector3D>;
		public function get_lastProfile() : Array<Vector3D>
		{
		 ...
		 */
		public var tweek(get, set) : Object;
		public function get_tweek() : Object
		{
			return _tweek;
		}
		
		public function set_tweek(val:Object) : Object
		{
			_tweek = val;
			invalidateGeometry();
		}
		...
		private function closeTopBottom(ptLength:Int, renderSide:RenderSide):Void
		{
			...
			// For loop conversion - 						for (i = 0; i < total; i += ptLength)
		...
		private function closeSides(ptLength:UInt, renderSide:RenderSide):Void
		{
			...
			// For loop conversion - 						for (i = 0; i < iter; ++i)
		...
		private function generate(vectors:Array<Vector3D>, axis:String, tweek:Object, render:Bool = true, id:UInt = 0):Void
		{
			// TODO: not used
			axis = axis;
			...
			if (!_varr)
				_varr = new Array<Vector3D>();
			
			// For loop conversion - 						for (i = 0; i < vectors.length; ++i)
			...
			// For loop conversion - 						for (i = 0; i <= lsub; ++i)
			
			for (i in 0...lsub) {
				
				tmpVecs = new Array<Vector3D>();
				tmpVecs = Lambda.array(vectors);
				
				// For loop conversion - 								for (j = 0; j < tmpVecs.length; ++j)
			...
			if (render) {
				...
				// For loop conversion - 								for (i = 0; i < loop; i += inc)
				
				for (i in 0...loop) {
					index = 0;
					// For loop conversion - 					for (j = 0; j < iter; ++j)
					for (j in 0...iter) {
		...
		{
			if (!_profile)
				throw new Error("LatheExtrude error: No profile Array<Vector3D> set");
			...
			if (_profile.length > 1) {
				...
				if (_thickness != 0) {
					...
					if (_preciseThickness) {
						...
						var profileFront:Array<Vector3D> = new Array<Vector3D>();
						var profileBack:Array<Vector3D> = new Array<Vector3D>();
						
						// For loop conversion - 												for (i = 0; i < lines.length; ++i)
						...
						var tmprofile1:Array<Vector3D> = new Array<Vector3D>();
						var tmprofile2:Array<Vector3D> = new Array<Vector3D>();
						var halft:Float = _thickness*.5;
						var val:Float;
						// For loop conversion - 						for (i = 0; i < _profile.length; ++i)
			...
			if (_MaterialsSubGeometries && _MaterialsSubGeometries.length > 0) {
				...
				var sg:SubGeometry;
				// For loop conversion - 				for (i = 1; i < 6; ++i)
				for (i in 1...6) {
		...
		private function buildThicknessPoints(aPoints:Array<Vector3D>, thickness:Float, prop1:String, prop2:String):Array<Dynamic>
		{
			var anchors:Array<Dynamic> = [];
			var lines:Array<Dynamic> = [];
			var i:Int;
			
			// For loop conversion - 						for (i = 0; i < aPoints.length - 1; ++i)
			...
			if (totallength > 1) {
				...
				// For loop conversion - 								for (i = 0; i < totallength; ++i)

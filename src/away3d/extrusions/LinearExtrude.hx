package away3d.extrusions;
	...
	class LinearExtrude extends Mesh
	{
		...
		var _thicknessSubdivision:UInt;
		var _ignoreSides:String;
		
		var _geomDirty:Bool = true;
		var _subGeometry:SubGeometry;
		var _MaterialsSubGeometries:Array<SubGeometryList>;
		var _uva:UV;
		var _uvb:UV;
		var _uvc:UV;
		var _uvd:UV;
		...
		public function new(material:MaterialBase = null, vectors:Array<Vector3D> = null, axis:String = LinearExtrude.Y_AXIS, offset:Float = 10, subdivision:UInt = 3, coverAll:Bool = false, thickness:Float = 0, thicknessSubdivision:UInt = 3, materials:MultipleMaterials = null, centerMesh:Bool = false, closePath:Bool = false, ignoreSides:String = "", flip:Bool = false)
		{
		...
		private function buildExtrude():Void
		{
			...
			if (_MaterialsSubGeometries && _MaterialsSubGeometries.length > 0) {
				...
				var sg:SubGeometry;
				// For loop conversion - 				for (var i:UInt = 1; i < _MaterialsSubGeometries.length; ++i)
				var i:UInt = 0;
		 ...
		 */
		public var thicknessSubdivision(get, set) : UInt;
		public function get_thicknessSubdivision() : UInt
		{
			return _thicknessSubdivision;
		}
		
		public function set_thicknessSubdivision(val:UInt) : UInt
		{
			val = (val < 3)? 3 : val;
		...
		private function generate():Void
		{
			...
			// For loop conversion - 						for (i = 1; i < _aVectors.length; i++)
			...
			if (_thickness != 0) {
				...
				// For loop conversion - 								for (i = 0; i < aLines.length; i++)
				...
				// For loop conversion - 								for (i = 0; i < _aVectors.length; i++)
			...
			// For loop conversion - 						for (i = 0; i < _aVectors.length - 1; ++i)
		...
		private function addThicknessSubdivision(points1:Array<Dynamic>, points2:Array<Dynamic>, u1:Float, u2:Float, mat:MaterialBase):Void
		{
			...
			// For loop conversion - 						for (i = 0; i < points1.length; ++i)
			...
			for (i in 0...points1.length) {
				...
				// For loop conversion - 								for (j = 0; j < _thicknessSubdivision + 1; ++j)
			...
			// For loop conversion - 						for (i = 0; i < points1.length - 1; ++i)
			
			for (i in 0...points1.length - 1) {
				
				// For loop conversion - 								for (j = 0; j < _thicknessSubdivision; ++j)
		...
		{
			// For loop conversion - 			for (var i:UInt = 0; i < _subdivision; ++i)
			var i:UInt = 0;
		...
		private function buildThicknessPoints(prop1:String, prop2:String):Array<Dynamic>
		{
			...
			// For loop conversion - 						for (var i:UInt = 0; i < _aVectors.length - 1; ++i)
		...
		private function definelines(index:UInt, point1:FourPoints, point2:FourPoints = null, lines:Array<Dynamic> = null):FourPoints
		{

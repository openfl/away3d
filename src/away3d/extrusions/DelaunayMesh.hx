...
package away3d.extrusions;
	...
	class DelaunayMesh extends Mesh
	{
		...
		var _circle:Vector3D;
		var _vectors:Array<Vector3D>;
		var _subGeometry:SubGeometry;
		var _sortProp:String;
		var _loopProp:String;
		 ...
		 *@param	vectors				Array<Vector3D> A series of vector3d's defining the surface of the shape.
		 ...
		 */
		public var vectors(get, set) : Array<Vector3D>;
		...
		private function addFace(v0:Vector3D, v1:Vector3D, v2:Vector3D, uv0:UV, uv1:UV, uv2:UV):Void
		{
			...
			if (_smoothSurface)
				var normals:Array<Float> = _normals;
			...
			if (_smoothSurface) {
				...
				if (indlength > 0) {
					...
					// For loop conversion - 										for (var i:UInt = indlength - 1; i > 0; --i)
					
					var i:UInt = 0;
					
					for (i in indlength - 1...0) {
						ind = indices[i];
						vind = ind*3;
						vindb = vind + 1;
						vindz = vind + 2;
		...
		private function generate():Void
		{
			...
			if (limit > 3) {
				...
				// For loop conversion - 								for (i = 0; i < (nv*3); ++i)
				...
				// For loop conversion - 								for (i = 0; i < maxTris; ++i)
				...
				// For loop conversion - 								for (i = 0; i < maxEdges; ++i)
				...
				// For loop conversion - 								for (i = 1; i < nv; ++i)
				...
				// For loop conversion - 								for (i = 0; i < nv; ++i)
				...
				for (i in 0...nv) {
					...
					// For loop conversion - 										for (j = 0; j < ntri; ++j)
					...
					// For loop conversion - 										for (j = 0; j < nEdge - 1; ++j)
					
					for (j in 0...nEdge - 1) {
						
						// For loop conversion - 												for (k = j + 1; k < nEdge; ++k)
					...
					// For loop conversion - 										for (j = 0; j < nEdge; ++j)
				...
				// For loop conversion - 								for (i = 0; i < ntri; ++i)
				...
				// For loop conversion - 								for (i = 0; i < v.length; ++i)
		...
		private function getVectorsBounds():Void
		{
			...
			switch (_plane) {
				case PLANE_XZ:
					_sortProp = "z";
					_loopProp = "x";
					// For loop conversion - 					for (i = 0; i < _vectors.length; ++i)

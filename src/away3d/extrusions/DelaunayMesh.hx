...
/**
    ...
    private function buildExtrude():Void {
        ...
        else throw new Error("DelaunayMesh: minimum 3 Vector3D are required to generate a surface");
...
// var yc:Number;
            ...
            while (i < nv) {
                ...
                while (j < nEdge - 1) {
                    k = j + 1;
                    while (k < nEdge) {
                        if ((edges[j].v0 == edges[k].v1) && (edges[j].v1 == edges[k].v0)) edges[j].v0 = edges[j].v1 = edges[k].v0 = edges[k].v1 = -1;
                        if ((edges[j].v0 == edges[k].v0) && (edges[j].v1 == edges[k].v1)) edges[j].v0 = edges[j].v1 = edges[k].v0 = edges[k].v1 = -1;
                        ++k;
                    }
                    ++j;
                }

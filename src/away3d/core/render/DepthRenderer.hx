package away3d.core.render;
	...
	class DepthRenderer extends RendererBase
	{
		...
		var _renderBlended:Bool;
		var _distanceBased:Bool;
		var _disableColor:Bool;
		...
		override function set_backgroundR(value:Float) : Float
		{
			return value;
		}
		
		override function set_backgroundG(value:Float) : Float
		{
			return value;
		}
		
		override function set_backgroundB(value:Float) : Float
		{
			return value;
		}
		
		public function renderCascades(entityCollector:EntityCollector, target:TextureBase, numCascades:UInt, scissorRects:Array<Rectangle>, cameras:Array<Camera3D>):Void
		{
			...
			var first:Bool = true;
			// For loop conversion - 			for (var i:Int = numCascades - 1; i >= 0; --i)
			var i:Int;
			for (i in (numCascades - 1)...-1) {

package away3d.core.managers;
	...
	class Mouse3DManager
	{
		...
		private static var _view3DLookup:Array<View3D>;
		...
		private static var _collidingViewObjects:Array<PickingCollisionVO>;
		private static var _queuedEvents:Array<MouseEvent3D> = new Array<MouseEvent3D>();
		...
		public function updateCollider(view:View3D):Void
		{
			...
			if (view!=null) {
				...
				if (view.stage3DProxy.bufferClear)
					_collidingViewObjects = new Array<PickingCollisionVO>();
		...
		public function fireMouseEvents():Void
		{
			...
			if (_collidingViewObjects!=null) {
				...
				var view:View3D;
				// For loop conversion - 				for (var v:Int = _viewCount - 1; v >= 0; v--)
				var v:Int;
				for (v in (_viewCount - 1)...0) {
			...
			_queuedEvents = new Array<MouseEvent3D>();
		...
		private function queueDispatch(event:MouseEvent3D, sourceEvent:MouseEvent, collider:PickingCollisionVO = null):Void
		{
			...
			if (collider==null) collider = _collidingObject;
		...
		private function traverseDisplayObjects(container:DisplayObjectContainer):Void
		{
			...
			// For loop conversion - 			for (c = 0; c < childCount; c++)
			for (c in 0...childCount) {
				child = container.getChildAt(c);
				var v:View3D;
				var cv:View3D = cast(child, View3D);
				for (v in _view3Ds.keys()) {
					if (cv == v) {
						_view3Ds.set(cv, _childDepth);

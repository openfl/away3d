...
/**
    ...
    public function distance(p:Vector3D):Float {
        if (_alignment == ALIGN_YZ_AXIS) return a * p.x - d
        else if (_alignment == ALIGN_XZ_AXIS) return b * p.y - d
        else if (_alignment == ALIGN_XY_AXIS) return c * p.z - d
        else return a * p.x + b * p.y + c * p.z - d;
    }
...
// check NaN
        ...
        var len:Float;
        if (_alignment == ALIGN_YZ_AXIS) len = a * p.x - d
        else if (_alignment == ALIGN_XZ_AXIS) len = b * p.y - d
        else if (_alignment == ALIGN_XY_AXIS) len = c * p.z - d
        else len = a * p.x + b * p.y + c * p.z - d;
        if (len < -epsilon) return PlaneClassification.BACK
        else if (len > epsilon) return PlaneClassification.FRONT
        else return PlaneClassification.INTERSECT;
    }

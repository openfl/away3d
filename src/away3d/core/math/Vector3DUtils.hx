...
// Extract the other two angles, rot.y and rot.z, from m2.
        var cy:Float = Math.sqrt(raw[(0)] * raw[(0)] + raw[(1)] * raw[(1)]);
...
// Fix angles
        if (Math.round(result.z / MathPI) == 1) {
            if (result.y > 0) result.y = -(result.y - MathPI)
            else result.y = -(result.y + MathPI);
            result.z -= MathPI;
            if (result.x > 0) result.x -= MathPI
            else result.x += MathPI;
        }
            ...
            result.x -= MathPI;
            if (result.z > 0) result.z -= MathPI
            else result.z += MathPI;
        }

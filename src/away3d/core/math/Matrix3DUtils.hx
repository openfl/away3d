package away3d.core.math;
	...
	class Matrix3DUtils
	{
		 ...
		 */
		public static var RAW_DATA_CONTAINER:Array<Float> = new Array<Float>();
		...
		public static function quaternion2matrix(quarternion:Quaternion, m:Matrix3D = null):Matrix3D
		{
			...
			var raw:Array<Float> = RAW_DATA_CONTAINER;
			raw[0] = 1 - 2*(yy + zz);
			raw[1] = 2*(xy + zw);
			raw[2] = 2*(xz - yw);
			raw[4] = 2*(xy - zw);
		...
		{
			if (v==null) v = new Vector3D(0.0, 0.0, 0.0);
			m.copyColumnTo(2, v);
			v.normalize();
		...
		{
			var r1:Array<Float> = Matrix3DUtils.RAW_DATA_CONTAINER;
			var r2:Array<Float> = m2.rawData;
			m1.copyRawDataTo(r1);
			
			// For loop conversion - 						for (var i:UInt = 0; i < 16; ++i)
